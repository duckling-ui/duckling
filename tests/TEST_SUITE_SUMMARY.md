...
- **Root (pytest)**: Lightweight repository checks and documentation structure tests in `tests/`
  ...
  - `tests/test_doc_screenshots.py`: Validates every localized doc screenshot reference resolves to an existing non-placeholder asset, uses absolute `/assets/` paths, prefers locale-specific PNGs when available, rejects blank export preview PNGs, and checks history-panel locale assets are structurally consistent
  - `tests/test_screenshot_automation.py`: Validates Playwright screenshot harness files, frontend `data-testid`/`data-section` hooks, SCREENSHOT_GUIDE Playwright documentation, localized index main screenshot paths, absolute `/assets/` gallery paths, and locale-specific PNG references (not SVG placeholders) for automated settings sections
