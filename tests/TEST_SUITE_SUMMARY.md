...
- **Root (pytest)**: Lightweight repository checks and documentation structure tests in `tests/`
  ...
  - `tests/test_get_version.py`: Ensures `scripts/get_version.py` updates only `extra.version.default` (does not corrupt i18n `fallback_to_default`) and docs workflows avoid broad `sed` on mkdocs.yml
