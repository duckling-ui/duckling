...
For local `buildx --load` builds (no `--push`), Buildx cannot export SBOM/provenance attestations via the Docker exporter; the script now warns and auto-disables `--sbom`/`--provenance` in that mode so the build still succeeds.
